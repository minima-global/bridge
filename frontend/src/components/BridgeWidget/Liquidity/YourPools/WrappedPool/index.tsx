...
const WrappedPool = () => {
  ...
  const handleDisable = () => {
    try {
      ...
      notify("Trading disabled for WMINIMA pool")
    } catch (error) {
    ...
    <Formik
      ...
      onSubmit={(values) => {
        try {
          ...
          notify("WMINIMA trading parameters updated successfully!")
        } catch (error) {
          if (error instanceof Error) {
            notify(error.message, "error")
          }
        ...
        <Form className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2 flex items-center justify-center">
              ...
              WMINIMA Trading Pool
            </h2>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Set your trading parameters for MINIMA/WMINIMA exchanges. This determines the prices at which you're willing to trade.
            </p>
          </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <InputField
                name="buy"
                label="Buy 1 MINIMA for (WMINIMA)"
                placeholder="0"
              ...
              <InputField
                name="sell"
                label="Sell 1 MINIMA for (WMINIMA)"
                placeholder="0"
                setFocused={setFocused}
                icon="./assets/wtoken.svg"
            ...
            <div className="bg-neutral-50 p-6 rounded-lg space-y-4">
              ...
              <div className="grid grid-cols-1 gap-6">
                <InputField
                  ...
                  icon={<div className="w-6 h-6 overflow-hidden rounded-full"><img src="./assets/token.svg" /></div>}
                />
                <div>
            ...
            {!!values.sell && !!values.buy && values.sell > 0 && values.buy > 0 && (
              <div className="bg-violet-100 dark:bg-violet-900 p-6 rounded-lg">
                ...
                <p className="text-sm text-violet-800 dark:text-violet-200 space-y-2">
                  ...
                  <span className="block">You would receive {new Decimal(100).dividedBy(values.buy).toFixed(2)} <strong>MINIMA</strong> for 100 <strong>WMINIMA</strong></span>
                  <span className="block">You would receive {new Decimal(100).times(values.sell).toFixed(2)} <strong>WMINIMA</strong> for 100 <strong>MINIMA</strong></span>
                </p>
              </div>
            )}
            ...
            {_allowanceLock ? (
              ...
              <div className="space-y-3">
                {!values.enable ? (
                  ...
                  >
                    {isValid ? "Enable WMINIMA For Trading" : "Please Fill All Fields Correctly"}
                  </button>
                ) : (
